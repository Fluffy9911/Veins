...
public class ModConfig {
	...
	public static class OreVeinConfig {
		...
		public HashMap<VeinType, ForgeConfigSpec.IntValue> veinGenerationFrequencies = new HashMap<>();
		...
		public OreVeinConfig(ForgeConfigSpec.Builder builder) {
			...
			builder.push("Vein Generation Frequency, smaller is more common");
			for (VeinType v : VeinType.values()) {
				ForgeConfigSpec.IntValue frequency = builder.comment("Generation frequency for " + v.name())
						.defineInRange(v.name() + "_generationFrequency", 15, 1, 9999);
			...
			for (VeinType v : VeinType.values()) {
				...
				vals.put(v,
						new Tuple<ForgeConfigSpec.IntValue, ForgeConfigSpec.IntValue>(
